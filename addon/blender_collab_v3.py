...
bl_info = {
    "name":        "BlenderCollab v3",
    "author":      "BlenderCollab",
    "version":     (3, 0, 0),
    "blender":     (4, 0, 0),
    "location":    "View3D › Sidebar › BlenderCollab",
    "description": "Site-plan aware collaboration with auto asset linking and GLB pipeline",
...
def _download_file(url: str, dest_path: str, token: str = "") -> None:
...
def _local_assets_dir(context) -> str:
    blend_path = bpy.data.filepath
    if blend_path:
        d = os.path.join(os.path.dirname(blend_path), "bc_assets")
    else:
        d = os.path.join(tempfile.gettempdir(), "bc_assets")
    os.makedirs(d, exist_ok=True)
    return d

def _collection_name(cell_id: str) -> str:
    return f"Cell_{cell_id.replace('.', '_')}"

def _remove_linked_cell(cell_id: str) -> None:
    """Remove linked objects + collection for a cell before re-linking."""
    coll_name = _collection_name(cell_id)
    for obj in list(bpy.data.objects):
        if obj.get("bc_cell") == cell_id and obj.get("bc_imported"):
            bpy.data.objects.remove(obj, do_unlink=True)
    for coll in list(bpy.data.collections):
        if coll.name == coll_name or coll.name.startswith(coll_name + "."):
            bpy.data.collections.remove(coll, do_unlink=True)
    for lib in list(bpy.data.libraries):
        stem = f"cell_{cell_id.replace('.', '_')}_assets"
        if stem in lib.filepath:
            bpy.data.libraries.remove(lib)


def link_cell_assets(context, cell_id: str, assets_path: str) -> bool:
    """
    Link the Collection Asset from assets_path into the scene at correct world origin.
    All objects in the collection are locked (read-only for other students).
    Returns True on success.
    """
    layout    = get_layout(context)
    cell_def  = layout.get("cells", {}).get(cell_id)
    if not cell_def:
        print(f"[bc] link_cell_assets: unknown cell {cell_id}")
        return False

    ox, oy    = cell_def["origin"]
    coll_name = _collection_name(cell_id)

    _remove_linked_cell(cell_id)

    with bpy.data.libraries.load(assets_path, link=True) as (data_from, data_to):
        if coll_name not in data_from.collections:
            print(f"[bc] Collection '{coll_name}' not found in {assets_path}")
            print(f"[bc] Available collections: {list(data_from.collections)}")
            return False
        data_to.collections = [coll_name]

    linked_coll = data_to.collections[0]
    if linked_coll is None:
        print(f"[bc] Linking returned None for {coll_name}")
        return False

    if linked_coll.name not in context.scene.collection.children:
        context.scene.collection.children.link(linked_coll)

    # Create library overrides so we can set position and lock transforms
    for obj in list(linked_coll.objects):
        try:
            ov = obj.override_create(remap_local_usages=True)
            ov.location.x  += ox
            ov.location.y  += oy
            ov["bc_cell"]      = cell_id
            ov["bc_imported"]  = True
            ov.lock_location   = (True, True, True)
            ov.lock_rotation   = (True, True, True)
            ov.lock_scale      = (True, True, True)
        except Exception as e:
            print(f"[bc] Override failed for {obj.name}: {e}")
            obj["bc_cell"]     = cell_id
            obj["bc_imported"] = True

    print(f"[bc] Linked cell {cell_id} from {assets_path} at ({ox},{oy})")
    return True
...
def _drain_sse_queue() -> float:
    ...
    while not _event_queue.empty():
        ...
        if event in ("cell_updated", "cell_pipeline_done"):
            cid = data.get("cell_id", "")
            _upsert_cell_state(s, cid, data)

            if cid != s.cell_id:
                # Mark changed for neighbour notification
                for cs in s.cell_states:
                    if cs.cell_id == cid:
                        cs.changed = True
                        break

                # Auto-link assets when the pipeline finishes for a neighbour
                if event == "cell_pipeline_done" and data.get("has_assets"):
                    neighbours = set(json.loads(s.neighbours)) if s.neighbours else set()
                    if cid in neighbours:
                        bpy.ops.bc.link_cell_assets(cell_id=cid)
...
class BC_OT_Publish(Operator):
    ...
    def execute(self, context):
        ...
        s.status_msg = (
            ...
            f"({n_excluded} excluded) | pipeline queued"
        )
        self.report({"INFO"}, s.status_msg)

        # Pipeline runs server-side (~10-30s). The SSE cell_pipeline_done event
        # will trigger auto-linking for neighbours automatically.
        # If assets are already ready (re-publish on fast machine), link now.
        if resp.get("has_assets"):
            bpy.ops.bc.link_cell_assets(cell_id=s.cell_id)
...
class BC_OT_LinkCellAssets(Operator):
    """Download _assets.blend for a cell and link its collection into the scene."""
    bl_idname  = "bc.link_cell_assets"
    bl_label   = "Link Cell Assets"
    ...
    @classmethod
    def poll(cls, context):
        return bool(context.scene.bc.token)

    def execute(self, context):
        s = bc(context)
        if not self.cell_id:
            self.report({"ERROR"}, "No cell_id provided")
            return {"CANCELLED"}

        # Check server knows this cell has assets
        cid = self.cell_id
        cell_state = next((cs for cs in s.cell_states if cs.cell_id == cid), None)

        assets_dir = _local_assets_dir(context)
        safe_id    = cid.replace(".", "_")
        local_path = os.path.join(assets_dir, f"cell_{safe_id}_assets.blend")

        url = f"{server_url(context)}/api/cells/{cid}/assets"
        try:
            _download_file(url, local_path, s.token)
        except urllib.error.HTTPError as e:
            if e.code == 404:
                self.report({"WARNING"}, f"Assets not ready yet for {cid} — pipeline still running")
            else:
                try:
                    detail = json.loads(e.read()).get("detail", f"HTTP {e.code}")
                except Exception:
                    detail = f"HTTP {e.code}"
                self.report({"ERROR"}, f"Download failed: {detail}")
            return {"CANCELLED"}
        except Exception as e:
            self.report({"ERROR"}, f"Download error: {e}")
            return {"CANCELLED"}

        ok = link_cell_assets(context, cid, local_path)
        if not ok:
            self.report({"ERROR"}, f"Could not link collection for {cid} — check console")
            return {"CANCELLED"}
        ...
        self.report({"INFO"}, f"Cell {cid} linked into scene")
        return {"FINISHED"}


class BC_OT_RelinkAllNeighbours(Operator):
    """Re-link all neighbour cells that have published assets."""
    bl_idname  = "bc.relink_all_neighbours"
    bl_label   = "Relink All Neighbours"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        s          = bc(context)
        neighbours = set(json.loads(s.neighbours)) if s.neighbours else set()
        cell_map   = {cs.cell_id: cs for cs in s.cell_states}
        linked     = 0
        skipped    = 0

        for nid in neighbours:
            cs = cell_map.get(nid)
            if cs and cs.published:
                op = bpy.ops.bc.link_cell_assets(cell_id=nid)
                if "FINISHED" in op:
                    linked += 1
                else:
                    skipped += 1
        ...
        self.report({"INFO"}, f"Linked {linked} neighbours, skipped {skipped}")
        return {"FINISHED"}
...
class BC_PT_Main(Panel):
    ...
    def draw(self, context):
        ...
        if s.token:
            ...
            assets = layout.box()
            assets.label(text="Neighbour Assets", icon="LINKED")
            assets.operator("bc.relink_all_neighbours", icon="FILE_REFRESH",
                            text="Relink All Neighbours")
    ...
    def draw(self, context):
        ...
        for cs in states:
            ...
            if cs.published and not is_me:
                # Use link_cell_assets (gets _assets.blend) if available,
                # fall back to raw download blend otherwise
                dl = row.operator("bc.link_cell_assets", text="↓ Link", icon="LINKED")
...
CLASSES = [
    ...
    BC_OT_Publish,
    BC_OT_DrawBounds,
    BC_OT_LinkCellAssets,
    BC_OT_RelinkAllNeighbours,
    BC_OT_DownloadCell,
    BC_OT_SetFilter,
    BC_OT_RefreshCells,
    BC_PT_Main,
